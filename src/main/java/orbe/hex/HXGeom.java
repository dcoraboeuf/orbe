...
public class HXGeom {
	...
	public static void line(HXPoint a, HXPoint b, boolean includea, boolean includeb, HXPointIterator iterator) {
		...
		while (xa != xb || ya != yb) {
			...
			if (dz >= 2) {
				// Un hex a été sauté, traiter l'hex supplémentaire
				// Hex courant
	...
	/**
	 * Calcul du sextant d'un point par rapport à l'origine
	 */
	...
	/**
	 * Parcours de a à b en suivant les contours des hex
	 * 
	 * @param a
	 *            Début du parcours
	 * @param b
	 ...
	 * @param x,
	 *            y, z Coordonnées normalisées du coin
	 * @param _012
	 *            Type de coin désiré. Valeur comprise entre 0, 1 et 2. Si coin
	...
	public static HX3DCorner to3DCorner(HXCorner c) {
		// Coordonnées normalisée de l'hex
		HX3D h = to3D(c.hex);
		// Calcul
		int x;
		int y;
